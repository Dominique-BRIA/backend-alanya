-- Plafond de participants d'une reunion — jumeau manuel de la migration
-- 20260821100000_limites_reunion.
--
-- POURQUOI CE FICHIER EXISTE. Le deploiement de ce projet ne joue PAS
-- `prisma migrate deploy` : il execute `scripts/apply-manual-sql.sh`, qui
-- applique `prisma/manual/*.sql` et rien d'autre. Toute migration deposee
-- seulement dans `prisma/migrations/` n'atteint donc JAMAIS la base de
-- production. Chaque migration du depot a son jumeau ici — celle-ci ne l'avait
-- pas, et le plafond de reunion est reste inapplique plusieurs jours pendant
-- qu'on croyait l'avoir livre.
--
-- Le contenu est identique a celui de la migration, deja ecrit en IF NOT
-- EXISTS et rejouable sans effet. Modifier l'un sans l'autre les fera diverger.
